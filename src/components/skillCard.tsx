...
export default function SkillCard({
	...
	useEffect(() => {
		const stateBase = Flip.getState(`#${baseID}`);
		const stateExpanded = Flip.getState(`#${expandedID}`);

		baseREF.current.classList.toggle("hidden");
		expandedREF.current.classList.toggle("hidden");

		if (isExpanded) {
			console.log(`Expanding ${skillTitle}`);
			Flip.from(stateBase, {
				target: stateExpanded,
				duration: 0.16,
				ease: "power1.inOut",
				zIndex: 40,
				fade: true, // Apply fade animation
				absolute: true, // Animate as if it's positioned absolutely
			});
		} else if (!isExpanded && expandedTitle !== null) {
			console.log(`Collapsing ${skillTitle}`);
			Flip.from(stateExpanded, {
				target: stateBase,
				duration: 0.16,
				ease: "power1.inOut",
				zIndex: 40,
